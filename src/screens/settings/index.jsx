...
import { getStorageConfig, setStorageConfig, STORAGE_MODES, isModeAllowed } from '../../lib/adapters/storageConfig'
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <BottomSheet isOpen={activeSection === 'backup'} onClose={() => setActiveSection(null)} title={t('settings.backupTitle')} large>
        <div className="space-y-5">
          <div className="card p-4">
            <p className="text-sm font-semibold text-gray-900 dark:text-gray-50 mb-3">{t('settings.storageMode')}</p>
            <p className="text-[11px] text-gray-500 mb-3">{t('settings.storageModeDesc')}</p>
            <div className="flex gap-2">
              {[
                { mode: 'local', label: t('settings.localMode') },
                { mode: 'cloud', label: t('settings.cloudMode') },
                { mode: 'hfsql', label: t('settings.hfsqlMode') },
              ].map(({ mode, label }) => {
                const current = getStorageConfig().mode
                const allowed = isModeAllowed(mode)
                return (
                  <button key={mode} onClick={() => {
                    if (!allowed) { toast.error(t('settings.storageNotAllowed')); return }
                    const result = setStorageConfig({ mode, setupComplete: true })
                    if (result.error) { toast.error(result.error); return }
                    toast.success(t('settings.storageChanged'))
                  }}
                    ...
                    } ${!allowed ? 'opacity-40 cursor-not-allowed' : 'active:scale-95'}`}>
                    {mode === 'local' ? '💻' : mode === 'cloud' ? '☁️' : '🗄️'} {label}
                  </button>
                )
              })}
            </div>
